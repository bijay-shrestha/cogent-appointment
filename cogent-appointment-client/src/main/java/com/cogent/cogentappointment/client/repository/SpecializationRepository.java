...
public interface SpecializationRepository extends JpaRepository<Specialization, Long>, SpecializationRepositoryCustom {
    ...
    @Query("SELECT s FROM Specialization s WHERE s.status='Y' AND s.id = :id AND s.hospital.id=:hospitalId")
    Optional<Specialization> findActiveSpecializationById(@Param("id") Long id,@Param("hospitalId") Long hospitalId);
